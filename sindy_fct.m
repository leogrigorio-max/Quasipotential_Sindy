function results = sindy_fct(file, target, makePlot)
%   SINDY_FCT Sparse identification of drift components from instanton data.
%
%   results = sindy_fct(file, target, makePlot) applies a Stepwise Sparse
%   Regressor (SSR) version of SINDy to instanton trajectory data.
%
%   The sparse-regression step solves
%
%       F ≈ Theta * Xi,
%
%   where F is the target vector field to be learned, Theta is the library
%   matrix of candidate functions, and Xi is the sparse coefficient matrix.
%
%   The method is used to identify components of the drift decomposition
%
%       b(x) = -grad V(x) + l(x),     p = 2 grad V(x).
%
%   If target = 'gradient', the code learns
%
%       F = -p/2,
%
%   corresponding to the gradient part of the drift. If target = 'transverse',
%   the code learns
%
%       F = dotX - p/2,
%
%   corresponding to the transverse component l(x).
%
%   The input .mat file is expected to contain:
%       x     : state trajectory, size 2 x N
%       p     : momentum trajectory, size 2 x N
%       dotX  : velocity along the instanton, size 2 x N
%
%   Inputs:
%       file     : path to the instanton .mat file
%       target   : 'gradient' or 'transverse'
%       makePlot : true/false, whether to show the cross-validation plot
%
%   Output:
%       results  : structure containing the selected sparse coefficients,
%                  cross-validation scores, and best sparsity level.
    ...
    % ==== choosing F=-p/2 to learn the gradient or F = dotX' - p'/2 to learn the orthogonal component
    switch lower(target)
        case 'gradient'
        ...
        otherwise
            error('target must be either ''gradient'' or ''transverse''.');
    end
...
fprintf('Best sparsity level: %d\n', j);
fprintf('Best validation score: %.6e\n\n', sc);
fprintf('\n  library   ||   F1 coeffs   ||   F2 coeffs \n')
disp([lib,best_Xi])
results = struct();
results.file = file;
results.target = target;
results.K = K;
results.tolerance = toler;
results.library = lib;
results.score = score;
results.final_score = final_score;
results.best_sparsity_level = j;
